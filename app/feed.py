...
async def _price(s, pair):
    kp = PAIRS[pair]["kraken"]
    try:
        async with s.get(f"https://api.kraken.com/0/public/Ticker?pair={kp}",
...
async def _candles(s, pair, interval=5, limit=150):
    kp = PAIRS[pair]["kraken"]
    try:
        async with s.get(f"https://api.kraken.com/0/public/OHLC?pair={kp}&interval={interval}",
...
async def fetch_fg(s):
    try:
        async with s.get("https://api.alternative.me/fng/?limit=1",
                         timeout=aiohttp.ClientTimeout(total=5)) as r:
            d = await r.json()
            with lock:
                state["news"]["fg"] = int(d["data"][0]["value"])
                state["news"]["fg_label"] = d["data"][0]["value_classification"]
    except: pass
...
async def stream():
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(limit=20)) as s:
        for pair in PAIRS:
            c5 = await _candles(s, pair, 5, 150)
            c1h= await _candles(s, pair, 60, 50)
            with lock:
        ...
        while True:
            try:
                prices = await asyncio.gather(*[_price(s, p) for p in PAIRS])
                with lock:
