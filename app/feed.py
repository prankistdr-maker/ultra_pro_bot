...
async def _c(session, pair_key, interval=5, limit=200):
    kp = PAIRS[pair_key]["kraken"]
    try:
...
async def stream():
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(limit=20)) as session:
        for pair in PAIRS:
            c5 = await _c(session, pair, 5, 200)
            c1h = await _c(session, pair, 60, 50)
            with lock:
        ...
        while True:
            try:
                prices = await asyncio.gather(*[_p(session, p) for p in PAIRS])
                with lock:
